    ...
    const unsubscribe = subscribe(
        ...
        (debuggerEnabled) => {
            ...
            timeoutRef.current = setTimeout(() => {
                if (
                    !debuggerEnabled.code ||
                    !debuggerEnabled.ast ||
                    !debuggerEnabled.blocklyJson ||
                    !debuggerEnabled.blocklyXml
                ) {
...
export function useWorkspace() {
    const { workspaceRef, isInitialized } = useContext(WorkspaceContext);
    return {
        workspace: workspaceRef.current!,
        isInitialized,
        load: (state: ISerializedWorkspace, target: Blockly.WorkspaceSvg = workspaceRef.current!) =>
            deserializeWorkspace(target, state),
...
>(id: T["id"] | undefined): T | undefined {
    ...
    return Object.entries(Blocks)
        .flatMap(([_, category]) => Object.values(category))
        .find((block) => block.id === id) as T | undefined;
}
...
export function getBlockDefinitionNameById(id: string | undefined): string | undefined {
    if (!id) return undefined;
    return Object.entries(Blocks)
        .flatMap(([_, category]) => Object.entries(category))
        .find(([_, block]) => block.id === id)?.[0];
}

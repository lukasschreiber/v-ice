...
import { ScopedBlock } from "../extensions/scoped"
...
interface ILocalVariableState {
    type: string
    scope: string
}

Blockly.Extensions.registerMutator(
    "local_variable_mutator",
    {
        saveExtraState: function (this: ScopedBlock): ILocalVariableState {
            const typeLabel = this.getField("TYPE") as FieldTypeLabel
            return { type: types.utils.toString(typeLabel!.getType()!), scope: this.scope }
        },
        loadExtraState: function (this: ScopedBlock, state: ILocalVariableState) {

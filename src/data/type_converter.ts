...
export class TypeConverter {
    static toType<T extends ColumnType>(value: unknown, type: T): ValueOf<T> {
        ...
        if(t.utils.isBoolean(type)) {
            return this.toBoolean(value, nullable) as ValueOf<T>;
        } else if(t.utils.isNumber(type)) {
            return this.toNumber(value, nullable) as ValueOf<T>;
        } else if(t.utils.isString(type)) {
            return this.toString(value, nullable) as ValueOf<T>;
        } else if(t.utils.isTimestamp(type)) {
            return this.toTimestamp(value, nullable) as ValueOf<T>;
        } else if(t.utils.isList(type)) {
            return this.toList(value, type, nullable) as ValueOf<T>;
        } else if(t.utils.isStruct(type)) {
            return this.toStruct(value, type, nullable) as ValueOf<T>;
        } else if(t.utils.isEnum(type)) {
            return this.toString(value, nullable) as ValueOf<T>;
        } else if(t.utils.isHierarchy(type)) {
            return this.toString(value, nullable) as ValueOf<T>;
        } else if(t.utils.isNull(type)) {
            return null as ValueOf<T>;
        } else if(t.utils.isUnion(type)) {

...
import {
    FibBlock,
    MathBinaryOperationBlock,
    MathDividedByBlock,
    MathMinusBlock,
    MathNumberPropertyBlock,
    MathPlusBlock,
    MathTimesBlock,
    MathUnaryOperationBlock,
} from "@/blocks/definitions/math";
import {
    CompareIntervalBlock,
    CompareNumbersBlock,
    EqualsBlock,
    EqualsWithinBlock,
    GreaterBlock,
    GreaterEqualsBlock,
    HasVariableValueBlock,
    IsNullBlock,
    LessBlock,
    LessEqualsBlock,
    MatchesBlock,
} from "@/blocks/definitions/comparisons";
import {
    ListAnyAllBlock,
    ListArithmeticBlock,
    ListContainsBlock,
...
import { FavoriteBlocks } from "./categories/favorite_blocks";

export const DefaultToolbox = buildToolbox()
    .addDynamicCategory(
        buildDynamicCategory("Favorites", "favorites_category")
            .withInstance(Favorites)
            .withMetadata({ noHighlight: true, sortable: false, filterable: false })
            .withCondition((_workspace, _table, settings) => !settings.toolboxFavorites)
            .build()
    )
    .addDynamicCategory(
        buildDynamicCategory("Favorite Blocks", "favorite_blocks_category")
            .withInstance(FavoriteBlocks)
            .withMetadata({ noHighlight: true })
            .withCondition((_workspace, _table, settings) => !settings.toolboxFavorites)
            .build()
    )
    .addDynamicCategory(
        buildDynamicCategory("%{BKY_VARIABLES}", "variables_category")
            .withInstance(Variables)
            .withMetadata({ filterable: true, sortable: true })
            .build()
    )
    .addStaticCategory(
        buildStaticCategory("%{BKY_COMPARISONS}", "comparisons_category")
            ...
            .addBlock(
                buildBlock(HasVariableValueBlock)
                    .withEmptyInputs()
                    .withCondition((_workspace, table) => {
                        return !table.some((col) => col.type.nullable);
                    })
    ...
    .addStaticCategory(
        buildStaticCategory("%{BKY_ARITHMETIC}", "math_category")
    ...
    .addStaticCategory(
        buildStaticCategory("%{BKY_LISTS}", "list_category")
    ...
    .addStaticCategory(
        buildStaticCategory("%{BKY_LOGIC}", "logic_category")
            .addBlock(buildBlock(LogicOrBlock).build())
            .addBlock(buildBlock(LogicNotBlock).build())
            .build()
    )
    .addStaticCategory(
        buildStaticCategory("%{BKY_TIMELINE}", "history_category")
            ...
            .addBlock(
                buildBlock(TimestampBlock)
                    .withInputs({
                        TIMESTAMP: {
                            shadow: buildBlock(TimelineDateBlock)
                                .withFields({
                                    TIMESTAMP: {
